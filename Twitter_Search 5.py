...
while i < len(search_word):
    ...
    #ツイート一覧から@を含むツイートを削除
    df_filter3 = df_filter2[df_filter2["ツイート"].str.contains("@") == False]
    
    #重複ユーザーを削除
    df_filter4 = df_filter3.drop_duplicates(subset = "ユーザー名")
  
    #df_filter4の表からユーザー名を抜き出し、ツイッタープロフィールのURLを作成する
    #まず、上記条件を満たしたユーザー名を表から抜き出しリスト化
    user_list = df_filter4["ユーザー名"].tolist()
    ...
    #df_filter4にプロフィールURLの列を追加し、csvファイルで出力
    df_filter4["プロフィールURL"] = user_url
    df_filter4.to_csv("result_twitter-{}.csv".format("[" + str(i) + "] " + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")))
